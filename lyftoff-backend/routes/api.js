const express = require('express');
   ...
   async function calculateReport(userId) {
     ...
     const profile = await retryQuery(() => Profile.findOne({ where: { userId }, include: [User] }));
     const roadmaps = await retryQuery(() =>
       Roadmap.findAll({
         where: { userId },
         include: [{ model: RoadmapStep, as: 'RoadmapSteps' }],
       })
     );
     const goals = await retryQuery(() => Goal.findAll({ where: { userId } }));

     const completedSteps = roadmaps.flatMap(r => r.RoadmapSteps || []).filter(s => s.completed).length;
     const totalSteps = roadmaps.flatMap(r => r.RoadmapSteps || []).length;
     const stepsWithDeadlines = roadmaps.flatMap(r => r.RoadmapSteps || []).filter(s => s.deadline).length;
   ...
   router.put('/roadmap/:id', authenticateToken, [
     ...
     try {
       ...
       if (updated) {
         const roadmap = await retryQuery(() =>
           Roadmap.findByPk(req.params.id, { include: [{ model: RoadmapStep, as: 'RoadmapSteps' }] })
         );
         cache.del(`roadmaps_${req.user.id}`);
         res.json(roadmap);
       } else {
   ...
   router.put('/roadmap/step/:id', authenticateToken, [
     ...
     try {
       ...
       if (req.body.completed) {
         await logAnalytics(req.user.id, 'step_completed', { stepId: step.id });
       }
   ...
   router.get('/notifications', authenticateToken, async (req, res) => {
     try {
       ...
       const notifications = roadmaps.flatMap(roadmap =>
         (roadmap.RoadmapSteps || []).filter(step => step.deadline && !step.completed).map(step => {
